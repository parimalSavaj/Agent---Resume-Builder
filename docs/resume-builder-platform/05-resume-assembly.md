...
- Links the finished resume version to the application's company record (see `07-company-records.md`), so it can be found again later by company name.
...
5. WHEN a resume version is finalized, THE SYSTEM SHALL link it to the corresponding company record.
