...
### ❌ Wrong: Flat files at module root instead of inside presentation/

```
modules/auth/
├── auth.controller.ts      ← wrong location — must be inside presentation/
├── auth.routes.ts          ← wrong location — must be inside presentation/
└── auth.schema.ts          ← wrong name and location — must be auth.validation.ts inside presentation/
```

### ❌ Wrong: Controller manually extracting fields from req

```ts
// WRONG: controller doing the DTO's job
...
### ✅ Correct: Controller delegates extraction to the DTO

```ts
// CORRECT
login = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const dto = LoginRequestDto.fromRequest(req);         // DTO owns extraction
...
### ❌ Wrong: Validation schema named `.schema.ts` or placed outside presentation/

```ts
// modules/auth/auth.schema.ts — WRONG name, WRONG location
```

### ✅ Correct: Validation file naming and placement

```ts
// modules/auth/presentation/auth.validation.ts — CORRECT
