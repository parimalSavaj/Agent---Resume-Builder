...
### ❌ Wrong: Business logic in a flat `auth.service.ts` instead of use cases

```ts
// modules/auth/auth.service.ts — WRONG
...
This is the "service" pattern — everything stuffed into one class. It is **not** how this architecture works.

### ✅ Correct: One use case file per operation
...
│   ├── login.dto.ts
│   └── register.dto.ts
├── login.use-case.ts       ← one class, one execute() method
└── register.use-case.ts    ← one class, one execute() method
```

### ❌ Wrong: DTO as a plain interface or type
...
### ✅ Correct: DTO as a class with static factories
...
export class LoginResponseDto {
  private constructor(...) {}
  static toResponse(entity: ...): LoginResponseDto { ... }
}
```
