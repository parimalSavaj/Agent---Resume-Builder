...
### ❌ Wrong: No factory, wiring done inside routes or controller

```ts
// auth.routes.ts — WRONG: creating concrete classes directly in routes
const repo = new UsersRepository(db);
const hashService = new HashService();
const jwtService = new JwtService();
const loginUseCase = new LoginUseCase(repo, hashService, jwtService);
const controller = new AuthController(loginUseCase);
```

### ❌ Wrong: Factory importing from the wrong layer

```ts
// auth.factory.ts — WRONG: importing concrete class from inside the module
import { AuthService } from './auth.service';          // ❌ no auth.service inside the module
import { AuthRepository } from './auth.repository';    // ❌ repository must come from infrastructure/
import { JwtService } from './jwt';                    // ❌ jwt must come from shared/services/jwt/
```

### ✅ Correct: Factory imports concrete classes only from infrastructure/ and shared/services/

```ts
// auth.factory.ts — CORRECT
import { UsersRepository } from '../../infrastructure/repositories/users/users.repository';        // ✅
import { RefreshTokensRepository } from '../../infrastructure/repositories/refresh-tokens/refresh-tokens.repository'; // ✅
import { JwtService } from '../../shared/services/jwt/jwt.service';                               // ✅ only if needed as concrete
